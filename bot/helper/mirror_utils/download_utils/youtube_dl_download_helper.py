...
class YoutubeDLHelper:
    ...
    def download_speed(self):
        with self.__resource_lock: return self.__download_speed
        ...
        if d['status'] == "finished":
            ...
            with self.__resource_lock:
                ...
                if self.is_playlist:
                    ...
                    try: self.size = d['total_bytes']
                    except KeyError:
                        if d.get('total_bytes_estimate'): self.size = d['total_bytes_estimate']
                    self.downloaded_bytes = d['downloaded_bytes']
                try: self.progress = (self.downloaded_bytes / self.size) * 100
    ...
    def __onDownloadComplete(self): self.__listener.onDownloadComplete()

    def __onDownloadError(self, error): self.__listener.onDownloadError(error)

    def extractMetaData(self, link, name, get_info=False):
        if get_info: self.opts['playlist_items'] = '0'
        with YoutubeDL(self.opts) as ydl:
            try:
                result = ydl.extract_info(link, download=False)
                if get_info: return result
        ...
        if 'entries' in result:
            for v in result['entries']:
                try: self.size += v['filesize_approx']
                except: pass
            self.is_playlist = True
            if name == "": self.name = str(realName).split(f" [{result['id']}]")[0]
            else: self.name = name
        else:
            ext = realName.split('.')[-1]
            if name == "": self.name = str(realName).split(f" [{result['id']}]")[0] + '.' + ext
            else: self.name = f"{name}.{ext}"
        ...
        try:
            with YoutubeDL(self.opts) as ydl:
                try: ydl.download([link])
                    ...
                    if not self.__is_cancelled: self.__onDownloadError(str(e))
                    return
            if self.__is_cancelled: raise ValueError
            self.__onDownloadComplete()
        except ValueError: self.__onDownloadError("Download Stopped by User.")

    def add_download(self, link, path, name, qual, playlist):
        if playlist: self.opts['ignoreerrors'] = True
        if "hotstar" in link or "sonyliv" in link: self.opts['geo_bypass_country'] = 'IN'
        self.__gid = ''.join(random.SystemRandom().choices(string.ascii_letters + string.digits, k=10))
        self.__onDownloadStart()
        if qual.startswith('ba/b'):
            audio_info = qual.split('-')
            qual = audio_info[0]
            if len(audio_info) == 2: rate = audio_info[1]
            else: rate = 320
    ...
    def cancel_download(self):
        ...
        if not self.__downloading: self.__onDownloadError("Download Cancelled by User.")
